...
void main(void) {
    ...
    float r = sqrt(rayoReflejado.x + rayoReflejado.y + rayoReflejado.z);
    float alfa = atan(rayoReflejado.y, rayoReflejado.x);
    float beta = acos(rayoReflejado.z, r);

    vec2 textureCoord = vec2(beta/(2*M_PI), alfo/M_PI);
    vec4 textureColor = texture2D(uSampler0,textureCoord);
